...
program define _dlw_api_v4, rclass
    ...
    if _rc > 0 & _rc != 110 {
        display as error "Unable to load the plugin from its central location. Please inform sol@worldbank.org."
        exit `= _rc'
    }
	...
	if (`option'==8) { //register token
		if "`token'"~="" & "`email'"~="" {			
			*local user = `=9-length("`user'")'*"0" + "`user'"
			cap plugin call _datalibweb_v4, "`option'" "`email'" "`token'"
			if _rc==0 {
		...
		if (`option'==6) { //user audit
			...
			if "`query'"~="" cap plugin call _datalibweb_v4, "`option'" "`outfile'" "`query'"
			//option 4 only
