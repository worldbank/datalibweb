			...
			foreach ctryx of local country {
				foreach yr of numlist `years' {
					foreach fld of global `request' {
						...
						cap noi _datalibcall_v4, country(`ctryx') year(`yr') type($type) token($token) vermast(`vermast') veralt(`veralt') folder(`fld') surveyid(`surveyid') para1(`para1') para2(`para2') para3(`para3') para4(`para4') $nocpi `fileserver' $nometa `base' `net' period(`period')
						global type_ `r(type)'
		...
		if "`latesty'"~="" {
			foreach ctryx of local country {
				...
				while `wrong'==1 {
					...
					cap noi _datalibcall_v4, country(`ctryx') year(`yr0') type($type) token($token) module(`module') vermast(`vermast') veralt(`veralt') filename(`filename') surveyid(`surveyid') para1(`para1') para2(`para2') para3(`para3') para4(`para4') ext($ext) $nocpi `fileserver' $nometa `base' `net' period(`period')
			...
			foreach ctryx of local country {
				foreach yr of numlist `years' {
					...
					cap noi _datalibcall_v4, country(`ctryx') year(`yr') type($type) token($token) module(`module') vermast(`vermast') veralt(`veralt') filename(`filename') surveyid(`surveyid') para1(`para1') para2(`para2') para3(`para3') para4(`para4') ext($ext) $nocpi `fileserver' $nometa `base' `net' period(`period')
					if $errcode==0 {					
						global type_ `r(type)'
						global module_ `r(module)'
						global verm_ `r(verm)'
