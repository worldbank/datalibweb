...
program define dlw_message
	...
	else if `error'==1 noi dis "{err}Available for download for subscribed users, files are too big. More than 1000MB. Split in server before downloading"		
	...
	else if `error'==673 noi dis "{err}Access denied - unregistered users - Please register with the datalibweb system"
	else if `error'==674 noi dis "{err}Access denied - registered users - no permission to the selected country/year/survey/collection"
	else if `error'==675 noi dis "{err}Not found - registered users - wrong country code/name. Please check the syntax - countrycode/year/collection/etc."
	else if `error'==676 noi dis "{err}Access denied - registered users - no permission to the selected Country/Year/Survey/Collection"
	else if `error'==677 noi dis "{err}Access denied - pending request"
	else if `error'==678 noi dis "{err}Access denied - expired subscription for the selected Country/Year/Collection"
	//Plugin error
	else if `error'==401 noi dis "{err}Error code 401 - Expired or invalid token"
	else if `error'==403 noi dis "{err}Error code 403 - Not an authorized user to download/not the same user"
