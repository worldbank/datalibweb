		...
		qui if ("`region'" != "" & "`code'" == "") {
			...
			foreach c of local codes {
				...
				noi disp _col(2) `"{stata datalibweb_inventory, code(`c') region(`region') : `c'}"'  ///
					in g _col(11) "{c |}" in y _col(`=66-length("`n'")') "`n' --> datalibweb_inventory, code(`c') region(`region')" 
			}	// end of codes loop	
		...
		*********************************************
		...
		qui if ("`region'" != "" & "`code'" != "" &  "`type'" == "") {	
			...
			foreach colraw of local colraws {
				...
				noi disp _col(6) in y "`colraw'" in g _col(19) "{c |}" _col(21) `"{stata `codeline': Data & documentation availability} --> `codeline'"'
			}
		...
		qui if ("`region'" == "" & "`code'" == "" &  "`type'" != "") {
			...
			foreach c of local codes {
				...
				noi disp _col(6) `"{stata datalibweb_inventory, code(`c') region(`r') type(`type') : `c'}"' ///
					in g _col(11) "{c |}" in y _col(`=37-length("`n'")') "`n' --> datalibweb_inventory, code(`c') region(`r') type(`type')" 
			}	// end of codes loop
			...
			* End Heading
			...
			foreach survey of local surveys {
				...
				foreach year of local years {
					...
					if `is_doc'>0 local codeq "{stata dlw, country(`code') year(`year') type(`type') surveyid(`survey') request(doc): Link}"
					else  local codeq " NA"
					if `is_doc'>0 local codet "{stata dlw, country(`code') year(`year') type(`type') surveyid(`survey') request(prog): Link}"
					else local codet " NA"
					*local codep "datalibweb, country(`code') year(`year') type(`type') surveyid(`survey') request(prog)"
					local codeb "dlw, country(`code') year(`year') type(`type') surveyid(`survey') clear"
