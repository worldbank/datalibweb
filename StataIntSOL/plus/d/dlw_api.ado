...
program define _dlw_api_v2, rclass
	...
	local pluginfile "dlib2sol_`=cond(strpos(`"`=c(machine_type)'"',"64"),64,32)'.dll"
	if "$S_OS" == "MacOSX" local pluginfile "dlib2sol_64.plugin"

	capture program define _datalibweb_v2, plugin using("`pluginfile'")	
    if _rc > 0 & _rc != 110 {
		display as error "Unable to load the plugin from its location, please check if `pluginfile' is copied to PLUS folder or no other plugin application is running."
        exit `= _rc'
    }
