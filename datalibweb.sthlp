...
{marker module}{...}
{synopt:{opt mod:ule(string)}} lists of modules to be merged and loaded. It is ONLY AVAILABLE for some collections such as {it:GMD}, {it:eappov}, ({it:ecapov} and {it:eusilc}), {it:SEDLAC}. It identifies the modules to be merged and loaded.
 Enter a list with the modules desired. If none is entered, {cmd:datalibweb} will load the default module defined by each type/collection, for example 3= Household Consumption for {it:ecapov} collection, and module D =Household Register for eusilc. {p_end}

					The following modules are available for the {it:GMD} collection:
						- GPWG
						- ALL
						
					For the {it:eusilc} collection, the following modules are available:
						- D: Household Register
						- H: Household
						- R: Personal Register
						- P: Personal

					For {it:ecapov} collection, the following modules are available:
					...
					For {it:eappov} collection, the following modules are available:
						- B: Basic data
						- H: Household 
						- I: Individual
						- POV: Poverty
...
{p 8 12}{stata "datalibweb, country(col) years(2008) type(gpwg) ppp(2011) incppp(welfare) clear" :.datalibweb, country(col) years(2008) type(gpwg) ppp(2011) incppp(welfare) clear}{p_end}
